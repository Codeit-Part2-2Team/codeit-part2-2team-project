...
이전 HPO에서 확인된 lr0=0.000645를 V2-S(24M)에 그대로 적용하면 모델 크기 차이로 인해 lr0가 상대적으로 과도하게 작용합니다. 초반 과수렴 후 지역 최솟값 고착이 발생했으며, lr0를 0.0003으로 재조정 후 성능이 개선됐습니다.
...
- [x] Stage 2 재현성 이슈 확인 완료 (한계점으로 기록 — D-4 일정상 코드 수정 보류)
