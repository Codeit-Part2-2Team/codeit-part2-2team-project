...
def main() -> None:
    ...
    parser.add_argument("--stage1-weights", default=None,  help="Stage 1 best.pt (미지정 시 config 자동 조합)")
    parser.add_argument("--stage2-weights", default=None,  help="Stage 2 best.pt (미지정 시 config 자동 조합)")
    parser.add_argument("--crop-output",    default="data/processed/crops/inference", help="crop 저장 디렉터리")
    parser.add_argument("--padding",        type=float, default=0.05, help="bbox 여백 비율 (기본값 0.05)")
    ...
    ], f"crop 생성: {crop_output}", cwd=ROOT)
    ...
    ], f"Stage 2 추론: {crop_output} → {stage2_predictions}", cwd=ROOT)
